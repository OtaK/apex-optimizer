...
pub fn apply_mousefix(pretend: bool) -> std::io::Result<()> {
    ...
    debug!(
        "Writing reg key: HKEY_CURRENT_USER\\Control Panel\\Mouse\\SmoothMouseXCurve = [binary]"
    );
    ...
    debug!(
        "Writing reg key: HKEY_CURRENT_USER\\Control Panel\\Mouse\\SmoothMouseYCurve = [binary]"
    );
    cu_mouse.set_raw_value(
        "SmoothMouseYCurve",
        &winreg::RegValue {
